...
- **Conversational Data Analysis:** Chat directly with your data using Gemini 3.1 Flash Lite — ask questions in natural language and get insights back.
- **Multi-Phase Analysis Pipeline:** Each query runs through a 4-phase pipeline: data extraction → code generation (with auto-retry) → execution → natural language interpretation. Falls back to text-only analysis if code execution fails.
- **Instant Interactive Visualizations:** Charts are generated via Plotly.js and rendered directly in the chat. Pin favorites to a persistent dashboard.
- **Smart Dataset Profiling:** Auto-detects dataset type (survey, time series, categorical, numerical) and generates tailored analysis suggestions.
- **Interactive Data Grid:** Full spreadsheet view of your dataset powered by Handsontable with column info toggles.
- **Dashboard Builder:** Pin charts from chat to a drag-and-drop dashboard grid with column spanning, reordering, and per-chart export.
- **Intelligent Caching:** LRU cache (300 queries) prevents redundant API calls. File schema and profile computations are also cached.
- **Secure Code Execution:** LLM-generated Python runs in an isolated subprocess with AST validation, module/attribute allowlists, 60-second timeouts, and code length limits.
